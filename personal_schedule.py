#!/usr/bin/env python3
"""
Personal Daily Schedule Notifier - Fixed Version with Wisdom & Mini-Game
"""
...
class PersonalScheduleNotifier:
    def __init__(self):
        ...
        self.wisdoms = [
            ...
            # ⚙️ Раздел 3. Эффективность, дисциплина и финансы
            ...
            # 🏗️ Раздел 6. Бизнес, мышление и реальность
            ...
            # ✍️ Раздел 7. Метафоры и жизненные истории
        ...
        # Personal schedule data с добавленными традициями и афформациями
        self.schedule = {
            'monday': {
            ...
            'sunday': {
                'утро': ['💝 Комплимент Марте и Саше', '🐱', '📵 Семейные традиции - у тебя День без гаджетов'],
    ...
    def get_random_wisdom(self):
        """Возвращает случайную мудрость из коллекции"""
    ...
    def get_current_period(self):
        """Определяет текущий период дня на основе времени"""
        now = datetime.now()
        ...
        if current_time == self.schedule_times['morning']:
            return 'morning'
        elif current_time == self.schedule_times['day']:
            return 'day' 
        elif current_time == self.schedule_times['evening']:
            return 'evening'
        elif 5 <= hour < 12:
            return 'morning'
        elif 12 <= hour < 17:
    ...
    async def send_appropriate_reminder(self):
        """Send the appropriate reminder based on current time"""
        period = self.get_current_period()
        date_str, day_of_week, schedule = self.get_today_schedule()
        
        logger.info(f"📋 Отправка {period} напоминания")
...
async def main():
    ...
    try:
        print("=" * 50)
        print("🚀 Personal Schedule Notifier - FIXED VERSION WITH WISDOM & MINI-GAME")
        print("=" * 50)
        
        notifier = PersonalScheduleNotifier()
        
        # Всегда отправляем соответствующее текущему времени сообщение
        success = await notifier.send_appropriate_reminder()
